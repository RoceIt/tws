...
from volwaptradingsystem import VolWapTestTrader
from fivestradingsystem import FiveSTestTrader
...
def main():
    ...
    p_trader = FiveSTestTrader(min_tick=min_tick)
    ...
    for announced, curr_base_bar, composing_bar, new_bar in new_bars:
        ...
        if new_bar:
            ...
            p_trader.new_five_s(new_bar, at_time=announced)
            #if new_triad:
    ...
    print (len(p_trader.active_requests),
           len(p_trader.finished_requests),
           len(p_trader.erroneous_requests))
