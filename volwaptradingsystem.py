...
class VolWapTestTrader(Trader):
    ...
    def new_volwapinfo(self, 
                vw_time, vw_vwap, vw_total, vw_mod_vw, vw_d_ratio, at_time):
        print('***********************', vw_mod_vw)
        if vw_mod_vw > 1:
            self.next_action = 'short'
        if vw_mod_vw < -1:
            self.next_action = 'long'
        if self.active_requests or self.next_action is None:
            return
        if ((self.next_action == 'short' and vw_mod_vw <= 0)
            or
            (self.next_action == 'long' and vw_mod_vw >= 0)):
            request = self.create_requests(self.next_action, at_time)
            if request:
