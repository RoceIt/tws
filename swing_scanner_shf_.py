...
import trade_simulator_for_perm_data_
...
def main():
    ...
    if True:  #mypy.get_bool('New analysis (Y/n)', default=True):
        bar_size = mypy.get_int('Bar size in seconds (60): ', default=60)
        #live_mode = mypy.get_bool('live mode (Y/n)', default=False)
        start_date = mypy.get_date('start from (2013/01/24): ', 
                                   format_='%Y%m%d', default='20130125')
        ...
        triad_list.use_bar_validation = True
        ...
        #trade_manager = trade_simulator_for_perm_data.TradeSimulationByArgs(
            #trailing_exit="entry+percent", start_trailing_at=1.7,
                                                                #sph=True)
        trade_manager = trade_simulator_for_perm_data_.TradeSimulationByArgs(
            #wavecount_in=2,
            wavecount_in=[2,4,6,8,10,12,14,16,18],
            #wavecount_in=5,
            trailing_exit="entry+percent", start_trailing_at=0.35, #0.12,
            #fix_exit="percent", fix_at=0.35,
            min_gap_to_curr="entry+fix", min_gap=0.07,
            #max_gap_to_curr="entry+fix", max_gap=2,
            cap_stop='entry+%', cap=0.14, #0.06,
                                                                sph=True)
        #trade_manager = trade_simulator_for_perm_data.TradeSimulationByRules(
                                                                #"first1hour.cr",
                                                                #sph=True)
