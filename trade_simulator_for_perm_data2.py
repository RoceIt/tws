...
class TraderRequest(list):
    
    def __init__(self, *ell):
        self.extend(ell)
        self.id_ = self[0]
        self.contract_name = self[1]
        self.direction = self[2]
        self.size = self[3]
        self.execution_time = self[4]
        self.price = self[5]
        self.reason = self[6]
        
    def __str__(self):
        t = r_string.SerialTextCreator()        
        ...
        t.add_chunk(str(self.contract_name))
        t.add_chunk('@')
        t.add_chunk(str(self.execution_time))
        return str(t)
...
class TradeSimulationByArgs(r_output.AddExportSystem):
    ...
    def __init__(self,
                 ...
                 sph=False, reverse=False):
        self.contract_name = contract_name
        self.stoploss_handler = stoploss_handler
        self.eod_handler = eod_handler
        ...
        self.positions = position_manager2.VirtualSinglePositions()
        self.last_send_report_info = None
        self.simulator_parameters_history = None if not sph else []
        super().__init__()
    
    @property
    def up_swing_start(self):
        return self.swing_start(self.up_swing_comp)
    
    @property
    def down_swing_start(self):
        return self.swing_start(self.down_swing_comp)
    
    @staticmethod
    def swing_start(swing_comp):        
        return swing_comp[0] if swing_comp else None
    
    @property
    def up_count(self):
        return len(self.up_swing_comp) 
    
    @property
    def down_count(self):
        return len(self.down_swing_comp)
            ...
            if not self.simulator_parameters_history == None:
                self.simulator_parameters_history.append(
                    (curr_time, perm_line[0] + str(perm_line[1]),
                     ...
                     self.up_swing_start, self.down_swing_start,
                     ))
        #for action in actions:
            #print('@#@: ',action)
        return actions 
        
    def virtual_register(self, perm_lines): 
        ...
        #curr_time = perm_lines[-1][-2]       
        for perm_line in perm_lines:
            if ((len(self.down_swing_comp) == 0 and
                 perm_line[0] == "down" and
                 not perm_line[1] == 0)
                or
                (len(self.up_swing_comp) == 0 and
                ...
                continue
            new_advise = self.virtual_register_perm_line(perm_line)
            if new_advise: advises.append((perm_line, new_advise))
    ...
    def register_perm_line(self, perm_line):
        ...
        if direction == 'up':
            if wavecount == 0: 
                ...
                self.up_swing_comp = self.up_swing_comp[:wavecount]
                self.up_swing_comp.append(triad_top_value)
            stop = self.swing_start(self.up_swing_comp)
            swing_comp = self.up_swing_comp
        else:
            if wavecount == 0: 
                self.down_swing_comp = [triad_top_value]
            else:
                self.down_swing_comp = self.down_swing_comp[:wavecount]
                self.down_swing_comp.append(triad_top_value)
            stop = self.swing_start(self.down_swing_comp)
            swing_comp = self.down_swing_comp
        if (not self.positions.in_trade() and            
            wavecount in self.wavecount_in):
            ...
            request = TraderRequest(0, self.contract_name, trade_direction, 1, 
                                    triad_exit_time, triad_exit_value, 
                                    'no trade: enter on wavecount {}'.format(
                                        wavecount),
                                    )
            self.positions.enter(0, self.contract_name, trade_direction, 1, 
                                 triad_exit_time, triad_exit_value, 
                                 'no trade: enter on wavecount {}'.format(
                                                           wavecount),
                                 stop)
            print('//////////////////exporting request (register): ', request)
            ...
            action = [0, self.contract_name, trade_direction, 1,
                      triad_exit_time, triad_exit_value, 
                      'no trade: enter on wavecount {}'.format(wavecount)]
        else:
            action = None
        return action
            
    def virtual_register_perm_line(self, perm_line):
        ...
        if direction == 'up':
            if wavecount == 0: 
                up_swing_comp = [triad_top_value]
            else:
                up_swing_comp = self.up_swing_comp[:wavecount]
                up_swing_comp.append(triad_top_value)
            stop = self.swing_start(up_swing_comp)
            swing_comp = up_swing_comp
        else:
            if wavecount == 0: 
                down_swing_comp = [triad_top_value]
            else:
                down_swing_comp = self.down_swing_comp[:wavecount]
                down_swing_comp.append(triad_top_value)
            stop = self.swing_start(down_swing_comp)
            swing_comp = self.down_swing_comp
        if (not self.positions.in_trade() and            
            wavecount in self.wavecount_in):
            # if not in trade or wavecount is not 
    ...
    def handle_eod(self, position, curr_ochl):
        actions = []
        if self.eod_handler == 'kiss':
            action = self.default_eod_handler(position, curr_ochl)
            if action:
                actions.append(action)
        else:
            raise Exception('Unknown eod handler')
        return actions
    
    def default_eod_handler(self, position, curr_ochl):
        id_ = position.id
        name, direction, size = self.positions.exit_parameters(id_)
        time_ = curr_ochl.time
        price = curr_ochl.open_
        reason = 'reached max daytime'
        request = TraderRequest(id_, name, direction, size, time_, price, reason)
        self.positions.exit(*request)
        print('//////////////////exporting request (eod handler): ', request)
        self.export_object(request)
    ...
    def default_stoploss_handler(self, position, curr_ochl):
        ...
        request = TraderRequest(id_, name, direction, size, time_, price, reason)
        self.positions.exit(*request)
        print('//////////////////exporting request (stplss handler): ', request)
        self.export_object(request)
        return [id_, name, direction, size, time_, price, reason]
    ...
    def default_profit_handler(self, position, curr_ochl):
        ...
        request = TraderRequest(id_, name, direction, size, time_, price, reason)
        self.positions.exit(*request)
        print('//////////////////exporting request (prft handler): ', request)
        self.export_object(request)
        return [id_, name, direction, size, time_, price, reason]
    
    def handle_eod_situation(self, curr_ochl):
        actions = []
        if self.positions.in_trade():
            for position in self.positions.current_postitions():
                actions.append(self.handle_eod(position, curr_ochl))
        return actions            
        ...
        if self.positions.in_trade():
            ...
            for position in self.positions.current_postitions():
                ...
                if ((position.direction == "long" and
                    ...
                    actions.append(self.handle_stoploss(position, curr_ochl))
        if self.down_swing_start and curr_ochl.high > self.down_swing_start:
            #self.short_stop = None
            #self.down_count = 0
            self.down_swing_comp = []
            reset = True
            #mypy.get_bool("resetting short")
        elif self.up_swing_start and curr_ochl.low < self.up_swing_start:
            #self.long_stop = None
            #self.up_count = 0
            self.up_swing_comp = []
            reset = True
    ...
    def live_bar_checks_and_actions(self, curr_ochl):
        actions = []
        eod_actions = []
        if self.day_trade_mode:
            ...
            if eod_actions_to_take:
                ...
                if DaytradeMode.CLOSE_ALL_POS_NOW in eod_actions_to_take:
                    #act = self.positions.exit_all_pos(
                            #curr_ochl.time, curr_ochl.close,
                            #"closing all positions, end of day")
                    #if act:
                        #input('bnnr')
                        #actions.append(act)
                    ###new
                    eod_actions = self.handle_eod_situation(curr_ochl)
                    ###
        if self.positions.in_trade() and self.positions.pending_enter:
            self.positions.check_pending_enter(curr_ochl)
        stop_actions = self.check_and_handle_stoplosses(curr_ochl)
        profit_actions = self.check_and_handle_profittakers(curr_ochl)
        ###new
        if eod_actions:
            actions.extend(eod_actions)
        if stop_actions:
                ...
                if position.out_type[0][0] == "trailing":
                    ...
                    if position.out_type[0][1] == "active":
                        ...
                        actions.append(
                                [position.id,
                                 finished_ochl.time, 
                                 'new trailing stop', position.stop])
        return actions
                            
    def report(self, 
               current_time, destinations, enter_advise=None, exit_advise=None):
        positions = self.positions.current_postitions()
        if 'sound' in destinations:
            self.sound_warning(
                current_time=current_time,
                positions=positions,
                enter_advise=enter_advise,
                exit_advise=exit_advise,
            )
        if 'terminal' in destinations:
            self.report_to_terminal(
                current_time=current_time,
                positions=positions,
                enter_advise=enter_advise,
                exit_advise=exit_advise,
            )
        self.last_send_report_info = (positions, enter_advise, exit_advise)
    
    def report_to_terminal(self, 
                           current_time, positions, enter_advise, exit_advise):
        t = r_string.SerialTextCreator()
        if enter_advise or exit_advise:
            t.add_box(str(current_time))
        else:
            t.add_line(str(current_time))
            t.underline()
        t.add_line('')
        if positions:
            for position in positions:
                t.add_lines(str(position))
        elif enter_advise:
            for advise in enter_advise:
                t.add_lines(str(advise))
        else:
            t.add_line('No positions')
            
        while t.nr_of_lines() < 19:
            t.add_line('')
        print(t)
        
    def sound_warning(self,
                      current_time, positions, enter_advise, exit_advise):
        if self.last_send_report_info:
            p_positions, p_enter_advise, p_exit_advise = (
                                            self.last_send_report_info)
        else:
            p_positions = p_enter_advise = p_exit_advise = []
        if not len(p_positions) == len(positions):
            r_output.enerve(level=4)
        if enter_advise or exit_advise:
            r_output.enerve(level=2)
...
#class TradeSimulationByRulesEM(TradeSimulationByArgs):
    ...
    #def text_output(self, *parali, **paradi):
        #self.positions.nn_output(*parali, **paradi)
