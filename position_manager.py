...
class Position():
    ...
    def __str__(self):
        ...
        t.add_chunk(self.direction.capitalize())
        t.next_line()
        t.add_chunk("reason:")
        t.add_chunk(self.reason)
        t.next_line()
        t.add_chunk("stop:")
        t.add_chunk(str(self.stop))
        t.add_chunk("          profit:")
        t.add_chunk(str(self.profit))
        t.next_line()
        t.add_chunk("in type:")
...
class VirtualSinglePositions():  
    ...
    def save_minimal_restart_info(self, filename, location=mypy.TMP_LOCATION):
        ...
        info = self.position
        mypy.export_pickle(info, outputfile, id_="trade_manager vsp: minimal_restart_info")
    ...
    def load_minimal_restart_info(self, filename, location=mypy.TMP_LOCATION):
        ...
        self.position = mypy.import_pickle(
            inputfile, id_="trade_manager vsp: minimal_restart_info")
        
    def save_action_log(self, filename, location=mypy.TMP_LOCATION): 
        outputfile = os.path.join(location, filename)
        info = self.action_log
        mypy.export_pickle(info, outputfile, id_="action_log")
        
    def load_action_log(self, filename, location=mypy.TMP_LOCATION):
        if not self.action_log == []:
            raise Exception("trying to load data in a running manager")
        inputfile = os.path.join(location, filename)
        self.action_log = mypy.import_pickle(inputfile, 
                                            id_="action_log")   
    ...
    def export_action_log(self, filename, location=mypy.TMP_LOCATION):
        ...
        with open(outputfile, 'w', newline='') as ofh:
            ...
            for action in self.action_log:
                csv_out.writerow(action)
                #action.to_csv_writer(csv_out)
    ...
    def export_positions(self, filename, location=mypy.TMP_LOCATION):      
        ...
        with open(outputfile, 'w', newline='') as ofh:
            if self.position:
                csv_out = csv.writer(ofh)
                self.position.to_csv_writer(csv_out)
            else:
                ofh.write("No possition")
    ...
    def exit_all_pos(self, time_, price, reason):
        ...
        self.exit(0, name, direction, size, time_, price, reason)
        return 1
