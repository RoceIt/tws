#!/usr/bin/env python3
#
#  Copyright (c) 2013, 2014 Rolf Camps (rolf.camps@scarlet.be)
#
...
LIVE_SERVER_PORT = 15501     # test value to stay away from production server
LIVE_SERVER_PORT = 14701     # !!!!!!!!!!!! hackje !!!!!!!!!!!!!!!!!!!



#print_ = print
#def print(*arg_l, **kw_d):
    #print_("MADA:", *arg_l, **kw_d)
    #input()
...
class DataBar(_DataBar):
    ...
    def __ge__(self, other):
        '''return True if high of self is bigger then or equal to high
        of other and low of self is bigger then or equal tolow of other
        '''
        ###
    ...
    def __gt__(self, other):
        ...
        and low of self is bigger then or equal to low of other'''
        ###
        greater_then = (
            self.high > other.high and
            self.low >= other.low
    ...
    def __le__(self, other):
        '''return True if high of self is lower then or equal to high
        of other and low of self is lower then or equal to low of other
        '''
        ###
        smaller_then_or_equal = (
            self.low <= other.low                                     and
            self.high <= other.high
        )
        ###
        return smaller_then_or_equal
    
    def __lt__(self, other):
        '''return True if high of self is lower then or equal to high
        of other and low of self is lower then low of other'''
        ###
        smaller_then = (
            self.low < other.low                                      and
            self.high <= other.high
    ...
    def high_low_equal(self, other):
        """return True if high of self is high of other and low of
        self is low of other
        """
        ###
        high_low_equal = (
            self.high == other.high                                   and
        ...
        ###
        return high_low_equal
    
    def spans(self, other):
        """return True if high and low of other are between or equal
        high and low of self.
        If other behaviour is excpected when bars have equal highs
        and lows, first check with high_low_equal.
        """
        ###
        spans = (
    ...
    @property
    def is_rising_bar(self):
        """Returns True if bar close is higher then bar open."""
        ###
        is_rising_bar = self.close > self.open_
        ###
        return is_rising_bar
    
    @property
    def is_falling_bar(self):
        """Returns True if bar close is lower then bar open."""
        ###
        is_falling_bar = self.close < self.open_
        ###
        return is_falling_bar
    ...
    def _add_bars(bar1, bar2):
        ...
        if not (bar1.counts == None or bar2.counts == None):
            counts = bar1.counts + bar2.counts or volume
        else:
            counts = None
        # duration & twap
...
class DataTick():
    ...
    def __init__(self, time_, value, size):
        """'Size' of contracts where traded for 'value' at 'time'."""
        self.time, self.value, self.size = time_, value, size
        
    def __str__(self):
        base = 'bought {size} of contracts for {value} at {time}'
        return base.format(**self.__dict__)
...
class DataBarComposer():
    ...
    def start_time(self):
        ...
        You can also enter a time object, the date of the first insered time
        will be used then.
        """
        ## TEST OK ##
        ###
        ###
        #try:
            #print('in starttime: self._start_time is', self._start_time)
        #except AttributeError:
            #pass
        #print('in starttime: returning ', getattr(self,'_start_time', None))
    ...
    def start_time(self, value):
        ...
        if value is None:
            return
        validate.as_date_or_datetime(value, 'start_time): {}')
        if self._composer_running():
            raise Error('can not change start_time of running composer')
        ###
        ###
        #print('in start time: setting self._start_time to: ', value)
        self._start_time = value
        if isinstance(value, datetime.time):
            self._starttime_is_time = value
    ...
    def pop_complete_bar(self):
        ...
        #print('*** pop complete bar for {}: '.format(self.composed_bar_duration), bar)
        return bar
    ...
    def reset_composer(self):
        ...
        answer = [x for x in self._work_list if x.duration]
        self._work_list = []
        if hasattr(self, '_starttime_is_time'):
            self._start_time = self._starttime_is_time
        else:
    ...
    def _insert_adjacent_bar(self, bar):
        ...
        if self.composing_bar.is_zero_bar():
            ...
            try:
                ...
                print('composing bar: ', self.composing_bar)
                print('bar to bar: ', bar)
                raise
    ...
    def _start_composer(self, time_, open_value):
        ...
        #print('starting composer with time', self.start_time,
              #'as start time')
        if isinstance(self.start_time, datetime.time):
            self.start_time = r_dt.timetodatetime(self.start_time,
                                                  time_.date())
            #print('changed start time to ', self.start_time)
        ...
        if self.start_time:
            ...
            ####################
            # TEST
            while start_time + self.composed_bar_duration <= time_:
                start_time += self.composed_bar_duration
            #
            ####################
        ...
        self._start_new_composing_bar_with_zero_bar(
            start_time, self._find_open_to_init_a_composing_bar(time_, 
                                                              open_value))
        if not time_ == start_time:
            self.move_time(time_, gaps=False)
    ...
    def _start_new_composing_bar_with_zero_bar(self, time_, open_):
        ...
        # Make sure you don't create a gap, new bar must be adjecent to previous
    ...
    def _fill_gaps_until(self, time_):
        ...
        ###
        #print('C456984356549876: filling until:', time_)
        #bar_end = self.end_time()
        #extend_time = bar_end if time_ > bar_end else time_
        ####
        ##print('extending composing bar to: {}'.format(extend_time))
        ##input('PROMPT!')
        #self._extend_composing_bar_until(extend_time, gaps=True)
            ...
            #self._fill_gaps_until(time_)
        extend_time = time_ - datetime.timedelta(1)
        while extend_time < time_:
            #print('C456984356549876: filling until:', time_)
            bar_end = self.end_time()
            extend_time = bar_end if time_ > bar_end else time_
            ###
...
def data_bar_feeder(*location, **kwds):
    ...
    if isinstance(location[0], list):
        ...
        feeder = DBFfromsql_ib_db_bars(*location, **kwds)
...
class DBFfromCSV_type2(DataBarFeeder):
    '''create DataBarFeeder from csv textfile.
    
    INPUT: Type2 cvs input, future tick data/second with volume
      header: True
      seperator: ,
      format: 19-02-2002,09:07:00,488.83,4
      time: second part of time (no milliseconds)      
    
    OUTPUT: Databar
      time, open_, high, low, close, volume, vwap in second bars      
    ...
    def __iter__(self):
        ...
        correct_volume = non_index_volume if not indexdata else index_volume
        last_datetime = None
        last_volume = last_counts = last_vwap = 0
        last_open = last_close = last_high = last_low = 0
        ###
        with open(self.__filename) as inf:
            skip_header = inf.readline()
            for line in inf:
                tick = self.line_to_tick(line)                
                if last_datetime and not last_datetime == tick.time:
                    databar = DataBar(
                            last_datetime,
                            last_open, last_high, last_low, last_close,
                            last_volume, last_counts, last_vwap,
                            duration=duration
                    )
                    yield databar
                    last_datetime = None
                if last_datetime is None:
                    last_datetime = tick.time
                    last_open = last_close = last_low = last_high = tick.value
                    last_volume, last_counts, last_vwap = tick.size, 1, tick.value
                else:
                    last_close = tick.value
                    last_low = min(last_low, tick.value)
                    last_high = max(last_high, tick.value)
                    last_counts += 1
                    prev_volume = last_volume
                    last_volume += tick.size
                    if not last_volume == 0:
                        last_vwap = (((prev_volume * last_vwap) 
    ...
    def line_to_tick(line):
        ...
        '''
        ###              
        date_, time_, value, size  = line.split(',')
        ...
        value = float(value)
        size = int(size)
        return DataTick(curr_datetime, value, size)
    
    @staticmethod                
    def check_file_format(*filename):
        ###
        filename = full_path(*filename)
        line_to_tick = DBFfromCSV_type2.line_to_tick
        with open(filename) as inf:            
            lines = (inf.readline(), inf.readline())
        ###
        try:
            line_to_tick(lines[1])
        except Exception as e:
            print('datalíne', lines)
            print(e)
            raise Error('wrong file format, dataline not ok')
        try:            
            line_to_tick(lines[0])
        except Exception:
            return True
        print('wrong?? header line')
        print(e)
...
class DBFfromCSV_type3(DataBarFeeder):
    '''create DataBarFeeder from csv textfile.
    ...
    known_headers = {
        'timestamp, open, low, high, close, volume, count, vwap, twap, gaps, 1 second'
    }
    ...
    def line_to_bar(line, volume_correction, duration):
        ...
        time = datetime.datetime.strptime(' '.join(x[:2]), 
                                          '%Y-%m-%d %H:%M:%S')
        open_, high, low, close, volume, count, vwap = (
            float(x[2]), float(x[3]), float(x[4]), float(x[5]), 
            volume_correction(x[6]),
            int(x[7]), float(x[8])
        )
        return DataBar(time, open_, high, low, close, volume, 
                       count, vwap, duration=duration)
    
    @staticmethod                
    def check_file_format(*filename):
        ###
        filename = full_path(*filename)
        line_to_bar = DBFfromCSV_type3.line_to_bar
        with open(filename) as inf:            
            lines = (inf.readline(), inf.readline())
        ###
        try:
...
class DBFfromsql_ib_db_bars(DataBarFeeder):
    ...
    info_dict = {
        'TRADES': DataBarFeederInfo('time', 'open_', 'high', 'low', 'close', 
                                    'volume', 'counts', 'vwap', 'hasgaps'),
        'MIDPOINT': DataBarFeederInfo('time', 'open_', 'high', 'low', 'close')
    }
    ...
    def __init__(self, *args, **kwds):
        ...
        barsize = kwds.pop('barsize', '5 secs')
        db_table = kwds.pop('db_table', 'TRADES_5_secs')
        show = kwds.pop('show', 'TRADES')
    ...
    def __iter__(self):            
        answer_to_bar_dict = {
            'TRADES': self.trades_answer_to_bar,
            'MIDPOINT': self.midpoint_answer_to_bar
        }
        def index_volume(foo):
            return None
        def non_index_volume(value):
            return int(value)
        ###
        duration = tws.rhd_bar_sizes_2_timedelta[self.barsize]#datetime.timedelta(seconds=5)
        indexdata = self.info.indexdata
        correct_volume = non_index_volume if not indexdata else index_volume
        ###
        if self.update_db:
            historicaldata.historical_data_to_db(
                ...
                barsize=self.barsize, #'5 secs', 
                show=self.show, #'TRADES', 
        ...
        #)
        inf_dict = {
            'TRADES': ['datetime', 'open', 'close', 'high', 'low',
                       'volume', 'counts', 'wap', 'hasgaps'],
            'MIDPOINT': ['datetime', 'open', 'close', 'high', 'low'],
        }
        #inf = db_h.data_stream(
                #self.db_table, #'TRADES_5_secs',
                #'datetime', 'open', 'close', 'high', 'low',
                #'volume', 'counts', 'wap', 'hasgaps',
                #start=self.start,
                #stop=self.stop,
        #)
        inf = db_h.data_stream(self.db_table, *inf_dict[self.show],
        ...
        while inf:
            #databar = self.answer_to_bar(
                        ...
                        #duration=duration)
            databar = answer_to_bar_dict[self.show](
                        answer=next(inf), 
                        volume_correction=correct_volume, 
                        duration=duration)
            yield databar
    ...
    @staticmethod
    def trades_answer_to_bar(answer, volume_correction, duration):
        '''Proces the line and return a Databar.
    ...
    @staticmethod
    def midpoint_answer_to_bar(answer, volume_correction, duration):
        '''Proces the line and return a Databar.
...
class DBFfromTWSLiveFeed(DataBarFeeder):
    ...
    def __init__(self, *args, **kwds):
        ...
        barsize = kwds.pop('barsize')  # !! ib_barsize
        show = kwds.pop('show')
...
FILE_TYPE_DATA_FEEDER_CLASS_TRANSLATION_DICT = {
   ...
   'csv_type2': DBFfromCSV_type2,
   'csv_type3': DBFfromCSV_type3,
}
...
class ComposingDatabarFeeder(r_output.AddExportSystem):
    ...
    def __init__(self, *args, **kwds):
        ...
        self.only_finished_bars = kwds.pop('only_finished_bars', False)
