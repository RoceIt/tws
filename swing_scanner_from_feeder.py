...
def main():
    ...
    if True:  #mypy.get_bool('New analysis (Y/n)', default=True):
        bar_size = mypy.get_int('Bar size in seconds (300): ', default=300)
        #live_mode = mypy.get_bool('live mode (Y/n)', default=False)
        #start_date = mypy.get_date('start from (2013/05/01): ', 
                                   #format_='%Y%m%d', default='20130501')
        ...
        swing_analyser = triads.SwingCountingByRules("cr_test.cr")
        #swing_analyser = triads.SwingCountingByArgs()
        ...
        #trade_manager = trade_simulator_for_perm_data.TradeSimulationByRules(
                                                                #"sist.cr",
                                                                #sph=True)
        #trade_manager = trade_simulator_for_perm_data.TradeSimulationByArgs(
            #trailing_exit="entry+percent", start_trailing_at=1.7,
                                                                #sph=True)
        trade_manager = trade_simulator_for_perm_data.TradeSimulationByArgs(
            #wavecount_in=2,
        ...
        #trade_manager = trade_simulator_for_perm_data.TradeSimulationByRules(
                                                                #"simpletrader.cr",
                                                                #sph=True)
        if mypy.get_bool("Exit end of day? (Y/n)", default=True):
            while True:
                try:
            ...
            last_entry = mypy.get_int("last entry (4500):",
                                      default = 4500)
            managed_exit = mypy.get_int("managed exit (sec before eod:",
                                        default = -1)
            forced_exit = mypy.get_int("force exit (1800):",
                                       default = 1800)
    ...
    data = {DB: None,
        ...
        #data[LIVE] = live_data
    #if start_date:
        #if mypy.get_bool("update db (Y/n)", default=True):
        ...
        #db_h = sql_ib_db.HistoricalDatabase(db_filename)
            ...
            #hist_data = db_h.data_stream(dataset_db, 
                                     ...
                                     #start=start_date)
        #data[DB] = hist_data
    #print('importing pickle data')
    #pd = mypy.import_pickle('/home/rolcam/roce/tmp.aex.db', 'serial databars')
    pd = mypy.import_pickle('/home/rolcam/roce/tmp.dax.db', 'serial databars')
    feeder = dbfeeder(pd)
    #feeder=dbfeeder('/home/rolcam/roce/Data/db/EUR CASH USD EUR.USD@IDEALPRO.db')
    data[DB] = feeder
    #for mode in [DB, LIVE]:
    for mode in [DB]:
        if not data[mode]:
        ...
        #for i, latest in enumerate(data[mode]):
            ...
            curr_ochl = ochlBar(bar.time, bar.open_, bar.close, bar.high, bar.low)
            
            curr_bar, new_base_bar = base_bar_list.insert(
                bar.time, bar.open_, bar.close, bar.high, bar.low)
    ...
    if True: #mypy.get_bool('Pickle last state for later use (y/N): '):
        ...
        info = ('feeder', filenames, bar_size, curr_bar.time)
...
def get_contract_info():
    ...
    menu.add_menu_item("Eurostoxx", return_value=(tws.contract_data("DJ_Eurostoxx50"),
