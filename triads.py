...
class TriadReducer():
    ...
    def insert(self, bar):
        ...
        if self.use_bar_validation and not self.valid_bar_p(bar): return None
    ...
    def valid_bar_p(self, bar):
        hour = bar.time.hour
        minutes = bar.time.minute
        if ((hour == 9 and minutes == 0)
            or
            (hour == 17 and minutes > 29)):
            #mypy.get_bool("{}:{}".format(hour, minutes))
            return False
        return True
    
    def save_minimal_restart_info(self, filename, location=mypy.TMP_LOCATION):
        outputfile = os.path.join(location, filename)
        info = (self.bar_size, self.bars, self.relative_bar_count,
                self.triad_list[-1:])
        mypy.export_pickle(info, outputfile, id_="triads: minimal_restart_info")
    ...
    def load_minimal_restart_info(self, filename, location=mypy.TMP_LOCATION):
        ...
        (self.bar_size, self.bars, self.relative_bar_count,
         self.triad_list) = mypy.import_pickle(
                                       inputfile, id_="triads: minimal_restart_info")
        
    def save_full_triad_history(self, filename, location=mypy.TMP_LOCATION): 
        outputfile = os.path.join(location, filename)
        info = self.triad_list
        mypy.export_pickle(info, outputfile, id_="full_triad_history")
        
    def load_full_bar_history(self, filename, location=mypy.TMP_LOCATION):
        if not self.triad_list == []:
            raise Exception("trying to load data in a running ochl")
        inputfile = os.path.join(location, filename)
        self.triad_list = mypy.import_pickle(inputfile, 
                                            id_="full_triad_history")        

    def export_triad_list(self, filename, location=mypy.TMP_LOCATION):
        outputfile = os.path.join(location, filename)
        with open(outputfile, 'w', newline='') as ofh:
            csv_out = csv.writer(ofh)
            for triad in self.triad_list:
                triad.to_csv_writer(csv_out)
    ...
    def last_triad(self):
        ...
        t_list = self.triad_list
        return t_list[-1] if t_list else None
    
    #@property
    #def last_move(self):
        #return self.last_move_look_back(1)
        
    #def last_move_look_back(self, look_back):
        #'''returns the difference between the last extrem and the
        #before last or between the look_back last and the one before
        #that one'''
        #if len(self.triad_list) >= look_back + 1:
            #last = self.triad_list[-look_back]
            #last_extreme = last.extreme_bar
            #before_last = self.triad_list[-look_back - 1]
            #before_last_extreme = before_last.extreme_bar
            #if before_last.type_ is Triad.DESCENDING:
                #diff_v = before_last_extreme.high - last_extreme.low
                #diff_l = before_last.close - last_extreme.low
                #diff_c = before_last.close - last.close
            #else:
                #diff_v = last_extreme.high - before_last_extreme.low
                #diff_l = last_extreme.high - before_last.close
                #diff_c = last.close - before_last.close
            #diff_t = last_extreme.time - before_last_extreme.time
            #diff_t += timedelta(seconds=self.bar_size)
        #else:
            #diff_v = diff_t = diff_l = diff_c =None
        #return diff_v, diff_t, diff_l, diff_c
    
    
class TriadReducerEM(TriadReducer):
    
    def insert(self, *parameters, finished=True):
        if not finished:            
            orig_bars = self.bars[:]
        new_triad = super().insert(*parameters)
        if new_triad:
            if finished and hasattr(self, "finished_triads_filename"):
                with open(self.finished_triads_filename, 'a', newline='') as ofh: 
                    writer = csv.writer(ofh)
                    new_triad.to_csv_writer(writer)
            if finished and hasattr(self, "easy_restart_filename"):
                self.save_minimal_restart_info(self.easy_restart_filename)
            if not finished:
                if hasattr(self, "unfinished_triad_file"):
                    with open(self.unfinished_triad_file, 'w', newline='') as ofh: 
                        writer = csv.writer(ofh)
                        new_triad.to_csv_writer(writer)
                mypy.export_pickle(new_triad, self.unfinished_triad_pickle,
                                   "triad")
                self.triad_list.pop()
        else:
            if not finished:
                open(self.unfinished_triad_file, 'w').close()
                open(self.unfinished_triad_pickle, 'w').close
        if not finished:
            self.bars = orig_bars
            self.relative_bar_count -= 1
    ...
    def hot_start(self, minimal_restart_info, mri_location=mypy.TMP_LOCATION,
                  full_triad_history=None, fth_location=mypy.TMP_LOCATION):
        ...
        if full_triad_history:
            self.full_triad_history_filename = full_triad_history
            self.full_triad_history_location = fth_location
            self.load_full_triad_history(full_triad_history, fth_location)
    ...
    def text_output(self, 
                    finished_triads_file=None, ft_location=mypy.TMP_LOCATION,
                    unfinished_triad_file=None, ut_location=mypy.TMP_LOCATION):
        if finished_triads_file == None and unfinished_bars_file == None:
            raise Exception("Live data mode started without files to export to")
        if finished_triads_file:
            self.finished_triads_filename = os.path.join(ft_location, 
                                                       finished_triads_file)
            if not hasattr(self, "_hot_start"):
                open(self.finished_triads_filename, 'w').close()
        if unfinished_triad_file == True:
            unfinished_triad_file = '.'.join([finished_triads_file, "unfinished"])
            ut_location = ft_location
        if unfinished_triad_file:
            self.unfinished_triad_file = os.path.join(ut_location,
                                                    unfinished_triad_file)
            self.unfinished_triad_pickle = '.'.join([self.unfinished_triad_file,
                                                    'p'])
...
class SwingCountingByArgs():
    ...
    def register(self, triad):
        ...
        def basic_uplist_counting():
            def append():
                up_list.append(triad)
                up_list_tops.append(triad.extreme_top)
            size_list = len(up_list)
            if size_list == 1:
                #mypy.get_bool("setting 2 in up", default=False)
                if triad_up:
                    raise Exception('two ups in a row?')
                append()
            elif size_list % 2 == 0:
                if triad_up:
                    if triad_top < up_list_min:
                        while up_list: 
                            up_list.pop()
                            up_list_tops.pop()
                        append()
                        ...
                        while len(up_list) > 2:
                            previous_top = up_list_tops[-2]
                            if triad_top >= previous_top:
                                break
                            if triad_top < previous_top:
                                up_list.pop(-2); up_list.pop(-2)
                                up_list_tops.pop(-2); up_list_tops.pop(-2)
                        append()
                else:
                    raise Exception('two ups in a row?')
            elif triad_down:
                if triad_top > up_list_max:
                    append()
            elif triad_up:
                if triad_top < up_list_min:
                    while up_list: 
        ...
        def basic_downlist_counting():
            def append():
                down_list.append(triad)
                down_list_tops.append(triad.extreme_top)
            size_list = len(down_list)
            ##print('in downlist with:')
            #print(down_list)            
            #mypy.get_bool("drukken", default=False)
            if size_list == 1:
                #mypy.get_bool("setting 2 in down", default=False)
                if triad_down:
                    raise Exception('two ups in a row?')
                append()
            elif size_list % 2 == 0:
                if triad_down:
                    if triad_top > down_list_max:
                        while down_list: 
                            down_list.pop()
                            down_list_tops.pop()
                        append()
                    else:
                        while len(down_list) > 2:
                            previous_top = down_list_tops[-2]
                            if triad_top <= previous_top:
                                break
                            if triad_top > previous_top:
                                down_list.pop(-2); down_list.pop(-2)
                                down_list_tops.pop(-2); down_list_tops.pop(-2)
                        append()
                else:
                    raise Exception('two ups in a row?')
            elif triad_up:
                if triad_top < down_list_min:
                    append()
            elif triad_down:
                if triad_top > down_list_max:
                    while down_list: 
                        ...
                        down_list_tops.pop()
                    append()
                elif triad_top > down_list_tops[-1]:                
                    down_list.pop()
                    down_list_tops.pop()
                    while len(down_list) > 2:
                        previous_top = down_list_tops[-2]
        ...
        def basic_counting():
            basic_uplist_counting()
            basic_downlist_counting()
            
        def special_reset(max_contratrend_triads):
            if len(up_list) == len(down_list):
                pass
            elif ((len(up_list) > len(down_list)) and
                (len(down_list) == max_contratrend_triads)):                
                while up_list: 
                    up_list.pop()
                    up_list_tops.pop()
                up_list.append(triad)
            elif ((len(up_list) < len(down_list)) and
                (len(up_list) == max_contratrend_triads)):                
                while down_list: 
                    down_list.pop()
                    down_list_tops.pop()
                down_list.append(triad)
        ...
        # tests
        triad_up = triad.type_ == Triad.ASCENDING
        triad_down = triad.type_ == Triad.DESCENDING
        # make it easy
        triad_top = triad.extreme_top
        down_list_tops = [t.extreme_top for t in down_list]
        up_list_tops = [t.extreme_top for t in up_list]
        if up_list_tops:
            up_list_max = max(up_list_tops)
            up_list_min = min(up_list_tops)
        if down_list_tops:
            down_list_max = max(down_list_tops)
            down_list_min = min(down_list_tops)        
        if not up_list and triad_up:
            up_list.append(triad)
            if down_list:
                down_list.append(triad)
        elif not down_list and triad_down:
            down_list.append(triad)
            if up_list:
                up_list.append(triad)
        else:
            ...
            if self.special_reset:
                special_reset(self.max_contratrend_triads)
        ...
        if ((uplist_size < 3) and (downlist_size < 3)):
            ...
            if downlist_size > 2: new_perms = downlist_perms() 
            if uplist_size > 2: new_perms.extend(uplist_perms()) 
        if new_perms:
    ...
    def save_minimal_restart_info(self, filename, location=mypy.TMP_LOCATION):
        outputfile = os.path.join(location, filename)
        info = (self.special_reset, self.max_contratrend_triads,
    ...
    def load_minimal_restart_info(self, filename, location=mypy.TMP_LOCATION):
        ...
        inputfile = os.path.join(location, filename)
        (self.special_reset, self.max_contratrend_triads,
         self.up_list, self.down_list, 
