...
def main():
    ...
    start_date = mypy.get_date('start from (2013/02/01): ', 
                               format_='%Y%m%d', empty=True)
    end_date = mypy.get_date('stop on: ', 
                             format_='%Y%m%d', empty=True)
    ...
    trade_manager = trade_simulator_for_perm_data_.TradeSimulationByArgs(
            ...
            wavecount_in=[5, 7, 9, 11, 13, 15, 17, 19],
            #wavecount_in=[3,5, 7, 9, 11, 13, 15, 17, 19],
            trailing_exit="entry+percent", start_trailing_at=1, #0.12,
            #fix_exit="percent", fix_at=0.10,
            #fix_exit="fix", fix_at=0.8,
            min_gap_to_curr="entry+fix", min_gap=0.15, #0.07,
            #max_gap_to_curr="entry+fix", max_gap=2,
            cap_stop='entry+%', cap=0.25, #0.06,
            #cap_stop='fix', cap=0.20,
                                                                sph=True)
        ##trade_manager = trade_simulator_for_perm_data.TradeSimulationByRules(
                                                                ##"first1hour.cr",
                                                                ##sph=True)
    ...
    if mypy.get_bool("Exit end of day? (y/N)", default=False):
        while True:
            try:
                eorth = input('How late do the markets close today (17:30:00)? ')
                eorth = '17:30:00' if not eorth else eorth
        ...
        ##for i, latest in enumerate(data[mode]):
            ...
            try:        
                ...
                if True: #mypy.get_bool('cont? (y)', default=True):
                    continue
                break
            #mypy.py_date_time(dt,'%d/%m/%Y %H:%M:%S')
            if end_date and curr_ochl.time > end_date: continue
            if start_date and curr_ochl.time < start_date: continue
