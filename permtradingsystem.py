...
#class PermsTradingSystem_0(TradingSystem):
    ...
    #def register_perm_line(self, perm_line, up_swing_comp, down_swing_comp):
        ...
        #request = None
            ...
            #)
        #return request
class PermBasicTestTrader(Trader):
    ...
    def create_requests(self, perm, at_time):
        ###
        ...
        exit_strategy = [
            ...
            strategies.SingleTrailingStop(
                ...
                start_trailing_percentage=0.20,
...
class PermBasicTestTraderDev(Trader):
    ...
    def create_requests(self, perm, at_time, reverse=False):
        ...
        exit_strategy = [
            strategies.SingleStopProfitTaker(
                stop_base='avg_parent_in',
                stop_percentage=0.07, #dax 0.25 beter
                #stop_ofset=15.5, # dax, 16.5 beter dan 15.5
                #stop_percentage=0.32,
                #stop_ofset=10,
                initial_stop_message='stopped out',
                profit_base='avg_parent_in',
                profit_percentage=0.07, #0.045, #dax
                #profit_ofset=4, #dax
            ...
            #strategies.SingleTrailingStop(
                ...
                #initial_stop_message='stopped out',
                #start_trailing_base='avg_parent_in',
                #start_trailing_percentage=0.07,
...
class KatiesOlTrader(Trader):
    ...
    def keterol_traded(self, action, at_time):
        if action[-1].startswith('no trade'):
            self.keterol_takes_a_position(action, at_time)
        else:
            self.keterol_closes_trade(action, at_time)
        
    def keterol_altered_trade(self, changes, at_time):
        print('keterol altered  trade @ {}: {}'.format(at_time, changes))
        
    def keterol_is_making_plans(self, plan, at_time):
        perm, plan = plan
        print('{} becaus of {} keterol plans {}'.format(at_time, perm, plan))
        if self.active_requests:
            return #pass #raise Exception('Kate in trade!')
        request = self.create_requests(plan, at_time)
        if request:
            request.id_ = 'keterol_{:0>8}'.format(next(self.id))
            self.add_requests([request], request.created)
            self.pre_in =True
        
    def a_new_bar_event_occurred(self, curr_trade_manager_positions, at_time):
        if self.pre_in and not curr_trade_manager_positions.in_trade():
            self.keterol_closes_trade("I'm in, kate nt", at_time)
        self.pre_in = False
        
    def keterol_takes_a_position(self, action, at_time):
        print('keterol opens @ {}: {}'.format(at_time, action))
        if self.pre_in:
            return
        if self.active_requests:
            pass #raise Exception('Kate in trade!')
        request = self.create_requests(action, at_time)
        if request:
            request.id_ = 'keterol_{:0>8}'.format(next(self.id))
            self.add_requests([request], request.created)
        
    def keterol_closes_trade(self, action, at_time):
        print('keterol closes @ {}: {}'.format(at_time, action))   
        self.set_signal_in_active_request('all', 'katie_stopped', at_time) 
       
    def create_requests(self, action, at_time):
        ###
        if self.last_in: #checking for daytrader settings part I
            date = self.last_official_timestamp.date()
            last_in = r_dt.timetodatetime(
